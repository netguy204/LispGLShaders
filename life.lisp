...
(defun neighbor-code (xvar yvar x y body)
  `(let ((,xvar ,x)
	 (,yvar ,y))
     (progn . ,body)))

(defmacro with-neighbors ((xvar yvar) b x y &body body)
  (let ((nx (gensym))
	(ny (gensym))
	(x-1 (gensym))
	(x+1 (gensym))
    ...
    `(let* ((,nx (num-cols ,b))
       ...
       ,(neighbor-code xvar yvar x-1 y body)
       ,(neighbor-code xvar yvar x+1 y body)
       ,(neighbor-code xvar yvar x y-1 body)
       ,(neighbor-code xvar yvar x y+1 body)
       ,(neighbor-code xvar yvar x-1 y+1 body)
       ,(neighbor-code xvar yvar x+1 y+1 body)
       ,(neighbor-code xvar yvar x-1 y-1 body)
       ,(neighbor-code xvar yvar x+1 y-1 body))))
...
(defmethod (setf boardref) (val (b board) x y)
  ...
  (with-neighbors (xn yn) b x y
    (add-item (changed b) (cons xn yn))))
